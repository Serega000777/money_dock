...
2. **Домен** — `amola-finance.ru` (куплен на reg.ru, DNS-серверы `ns1.hosting.reg.ru` /
   `ns2.hosting.reg.ru`). Нужна A-запись корня домена на IP сервера. Подождите, пока
   `nslookup amola-finance.ru` отвечает новым IP, иначе Let's Encrypt не выдаст сертификат.
...
- **Домен:** в карточке `amola-finance.ru` → управление DNS-зоной. Домен делегирован на
  `ns1.hosting.reg.ru`/`ns2.hosting.reg.ru`, так что зона редактируется в панели reg.ru
  (раздел DNS у домена или у хостинга — там, где перечислены записи). Добавьте запись:
  тип **A**, имя **`@`** (корень домена), значение — IP сервера, TTL по умолчанию. Если
  там уже стоит A-запись на парковку/хостинг reg.ru — замените её. Обновление зоны
  занимает обычно 5–15 минут; проверка — `nslookup amola-finance.ru` должен вернуть IP.
- `www.amola-finance.ru` для Mini App не нужен; если позже захочется — вторая A-запись
  `www` на тот же IP и одна строка в Caddyfile с редиректом.
...
1. `/newapp` → выберите @amola_finance_bot → название «Amola Finance», короткое описание,
   картинка 640×360 → **Web App URL: `https://amola-finance.ru`** → короткое имя `amola`.
   Прямая ссылка на Mini App: `https://t.me/amola_finance_bot/amola`.
2. `/setmenubutton` → @amola_finance_bot → URL `https://amola-finance.ru` → текст кнопки,
