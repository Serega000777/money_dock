...
Цель («Отпуск — 200 000 ₽») и сколько на неё уже отложено. Это трекер, а не движение
денег: `contribute` меняет только `savedMinor`, операция не создаётся — деньги и так лежат
на одном из счетов пользователя, цель их лишь «помечает».
...
Тело: `{ name, icon?, targetMinor, currency, deadline? }` — `icon` одно из имён иконок
приложения (`GOAL_ICONS` в `apps/app/src/features/goals.tsx`), `deadline` — `YYYY-MM-DD`.

### `POST /goals/:id/contribute`

Тело: `{ amountMinor }` — положительное «отложить», отрицательное «снять». Ноль → 400;
результат ниже нуля → 400 (`Нельзя снять больше, чем отложено`). Возвращает обновлённую цель.

### `DELETE /goals/:id` → 204

Чужая цель по любому из этих маршрутов — 404.
