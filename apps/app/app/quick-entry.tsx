...
export default function QuickEntry() {
  ...
  return <><Stack.Screen options={{ headerShown: true, title: "Быстрый ввод" }} /><Screen>
    ...
    <Card style={styles.card}>
      ...
      <Text style={[styles.body, { color: theme.textSecondary }]}>1. Установите бесплатное приложение «HTTP Shortcuts» (Google Play или F-Droid, автор — Roland Meyer) — оно делает то же самое, что «Команды» на iPhone.{"\n\n"}2. Создайте новый шорткат: тип запроса — Text Request, метод — POST, вставьте адрес ниже.</Text>
      <Text style={[styles.label, { color: theme.textTertiary }]}>URL</Text><Text selectable style={[styles.code, { color: theme.textPrimary }]}>{API_URL}</Text>
      <PressableScale style={StyleSheet.flatten([styles.button, { backgroundColor: theme.accent }])} onPress={() => copy(API_URL)}><Text style={{ color: theme.onAccent }}>Скопировать URL</Text></PressableScale>
      <Text style={[styles.body, { color: theme.textSecondary }]}>3. В заголовках (Headers) добавьте Authorization со значением Bearer и вашим ключом через пробел.{"\n\n"}4. В теле запроса (Request body, тип JSON) вставьте образец ниже — сначала создайте переменную типа «Text Input» с диалогом ввода, вставьте её в поле input через кнопку {"{x}"} у поля.</Text>
      <Text style={[styles.label, { color: theme.textTertiary }]}>Тело запроса</Text>
      <Text selectable style={[styles.code, { color: theme.textPrimary }]}>{`{"input": "{{ваша переменная}}", "mode": "text"}`}</Text>
      <Text style={[styles.body, { color: theme.textSecondary }]}>5. Сохраните и добавьте значок на главный экран (пункт меню «Place on home screen»).{"\n\n"}Дальше: тапнули значок → откроется поле ввода → нажмите значок микрофона на клавиатуре Android и продиктуйте, например «Кофе 340 рублей» → подтвердите. Операция появится в Amola и разделе проверки. Telegram открывать не нужно, интернет — нужен. clientRequestId в теле запроса можно не указывать — при повторном одинаковом запросе сервер один раз создаст новую операцию сам (без него теряется только защита от случайного дубля при повторной отправке того же самого запроса).</Text>
    </Card>
    <Card style={styles.card}>
